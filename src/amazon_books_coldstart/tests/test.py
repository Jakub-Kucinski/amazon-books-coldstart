...
class Test():
    def __init__(self, dataset_type):        
        self.books = pd.read_csv("data/02_intermediate/" + dataset_type + "_books.csv")[['book_id', 'authors', 'publisher', 'categories']]
        self.ratings = pd.read_csv("data/02_intermediate/" + dataset_type + "_ratings.csv")
    ...
    def test_books_helper(self, get_answer, get_number_of_users):
        ...
        for index, book in tqdm(self.books.iterrows()):
            ...
            recommended_users = get_answer(book['authors'], book['publisher'], book['categories'], get_number_of_users(book_id))
        ...
        print_stats("books below threshold", precision_below_threshold, recall_below_threshold, books_below_threshold)
        print_stats("book on and above threshold", precision_above_threshold, recall_above_threshold, books_above_threshold)
