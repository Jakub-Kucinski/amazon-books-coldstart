...
class Data_reader:
    ...
    def read_ratings(self):
        i = 0
        success = 0
        with open("../../data/01_raw/Books_rating.csv", newline="") as csv_file:
            spam_reader = csv.reader(csv_file, delimiter=",", quotechar="|")
            for row in spam_reader:
                if i > 0:
                    try:
                        review = Review(row)
                        if str(row[0]) not in self.ratings.keys():
                            self.ratings[str(row[0])] = []
                        self.ratings[str(row[0])].append(review)
    ...
    def read_books(self):
        i = 0
        success = 0
        with open("../../data/01_raw/books_data.csv", newline="") as csv_file:
            spam_reader = csv.reader(csv_file, delimiter=",", quotechar="|")
            for row in spam_reader:
                if i > 0:
                    try:
    ...
    def split_dataset(self, train_size, valid_size):
        ...
        all_indices = np.random.choice(
            list(self.title_to_id.values()), size=len(self.title_to_id), replace=False
        )
